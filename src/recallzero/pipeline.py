...
class RecallZeroPipeline:
    ...
    async def extract_signatures(
        ...
        *,
        ...
        signatures = await self.extractor.extract_many(complaints, cached=cache)
        merged = dict(cache)
        merged.update({signature.complaint_id: signature for signature in signatures})
    ...
    async def analyze_records(
        ...
        *,
        ...
        clusters, embedding = await self.clusterer.cluster(visible_complaints, visible_signatures)
        ...
        for cluster in clusters:
            ...
            recall_match = self.recall_matcher.find_best_match(cluster, member_signatures, visible_recalls)
...
def build_pipeline(
    ...
    *,
    ...
    embedder = HybridEmbedder(nim=nim_embedder, fallback=TFIDFEmbedder())
    clusterer = ComplaintClusterer(
        embedder=embedder,
